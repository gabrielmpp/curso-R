...
modLinear <- lm(Nebulosidade ~ UmidadeRelativa, data = df_lm)
...
Volte na tabela de correlação e compare o coeficiente de pearson com o R².  
  
É possível plotar a reta da regressão linear:  
  
```{r}
plot(x = df_lm$UmidadeRelativa, y = df_lm$Nebulosidade, xlim = c(0,2), ylim = c(0,2)) 
abline(a = modLinear$coefficients[1], b = modLinear$coefficients[2], col = "red")
```

O gráfico ficou horrível! Isso acontece pois a nebulosidade é uma variável não-contínua. Os valores são categóricos (discretos), mas possuem um sentido de ordem, sendo 0 o caso sem nuvens e 1 a situação overcast. Portanto, embora não seja a melhor opção, é possível obter alguma informação através da regressão linear.  
...
Reproduza os plots da parte 3 usando o ggplot2 e pense em um novo plot baseado nas ideias do top50.

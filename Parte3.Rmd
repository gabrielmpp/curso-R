...
nc <- nc_open("/media/gabriel/Novo volume/Mestrado/chirpsMonthlyNCDF/chirps_AS/chirps015.nc")
...
```{r}
levelplot(chirps_AS[[1]],margin=FALSE)
```
...
```{r}
levelplot(chirps_AS[[1]],margin=FALSE,par.settings=RdBuTheme)
...
```{r}
levelplot(chirps_AS[[1]],margin=FALSE,par.settings=RdBuTheme)+
  contourplot(chirps_AS[[1]],at=c(50,200,500,800))
...
```{r}
library(rgdal)
chirps_AS_LowRes <- aggregate(chirps_AS,factor=2)
